...
void TransportCatalogue::AddRoute(const std::string& num, const std::vector<std::string>& stops, const std::vector<std::shared_ptr<Stop>> ptrs, RouteType type){
     all_buses.push_back({num, stops, ptrs, type});
...
std::tuple<int,int,int,double> TransportCatalogue::RouteInfo(const std::string& num) const{
     ...
     if (bus.get()->type == RouteType::CIRCLE){
    ...
    //считаем длину и курватуру
     ...
     for (size_t i = 1; i < bus.get()->stops.size(); ++i) {
        ...
        if (bus.get()->type == transport::RouteType::CIRCLE) {
