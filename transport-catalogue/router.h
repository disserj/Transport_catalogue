...
private:
    ...
    /*// внутренние поля: шаблонный вес
                         орграф
                         матрица из веса и предыдущей грани
...
public:
    ...
    //состоит из веса и списка номеров граней 
    struct RouteInfo {
        Weight weight;
...
private:
    struct RouteInternalData {
        Weight weight;// вес
        std::optional<EdgeId> prev_edge; // предыдущая грань/ее отсутствие
    };
    //???? матрица из веса и предыдущей грани
    ...
    void InitializeRoutesInternalData(const Graph& graph) {
        ...
        for (VertexId vertex = 0; vertex < vertex_count; ++vertex) {
            ...
            for (const EdgeId edge_id : graph.GetIncidentEdges(vertex)) {
                ...
                //если грань есть, матрица с этой вершиной инициализируется номером вершины "в"
...
// ослабление дуги
    void RelaxRoute(VertexId vertex_from, VertexId vertex_to, const RouteInternalData& route_from/*вес+грань*/,
...
// маршрут построен
//возвращает //структуру, состоящую из веса и списка номеров граней либо nullopt     
template <typename Weight>
