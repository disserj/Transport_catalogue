...
void Read(std::istream& in, Request_Queue& request) {
		...
		if(IsDigit(line)) count = std::stoi(line);
		request.ToFill.resize(count);
		for (int i = 0; i < count; i++) {
			std::string l;
			if (!std::getline(in, l)) return;
            if(!l.empty()){
            l = l.substr(l.find_first_not_of(' '),
                             l.find_last_not_of(' ') - l.find_first_not_of(' ') + 1 );
            }
			request.ToFill[i] = std::move(l);
		}
		if (!std::getline(in, line)) return;
		if(IsDigit(line)) count = std::stoi(line);
		...
		for (int i = 0; i < count; i++) {
			...
			request.ToResponse[i] = std::move(l);
		}
}
...
std::pair<std::string,std::string> FillStops(std::string& str, TransportCatalogue& catalogue){
    ...
    if(str.find_first_of(',') == str.npos) str.clear();
    else str.erase(0, str.find_first_of(',')+2);
