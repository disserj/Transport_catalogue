...
svg::Document MapBuilder::BuildSvg(const transport::TransportCatalogue& catalogue){
    ...
    const std::map<std::string_view, transport::Bus*> list_buses = catalogue.GetBusCatalogue();
    std::map<std::string_view, transport::Stop*> list_stops;
    std::vector<geo::Coordinates> stops_crd;
    
    for (const auto& [bus_number, bus] : list_buses) {
        if (bus->GetStopsPtrs().empty()) continue;
        for (const auto& stop : bus->GetStopsPtrs()) {
...
std::vector<svg::Polyline> MapBuilder::RouteOutline(const transport::TransportCatalogue& catalogue, const SphereProjector& sp) const{
    ...
    for (const auto& [bus_number, bus] : list_buses) {
        if (bus->GetStopsPtrs().empty()) continue;
        std::vector<transport::Stop*> route_stops= bus->GetStopsPtrs();
        if (bus->cycle == false ) {
...
std::vector<svg::Text> MapBuilder::RenderBusLabel(const transport::TransportCatalogue& catalogue, const SphereProjector& sp) const {
    ...
    size_t color_num = 0;
    for (const auto& [bus_number, bus] :list_buses) {
        std::vector<transport::Stop*> route_stops_ptrs = bus->GetStopsPtrs();
...
std::vector<svg::Circle> MapBuilder::RenderStopsCircles(const std::map<std::string_view, transport::Stop*>& list_stops, const SphereProjector& sp) const {
...
std::vector<svg::Text> MapBuilder::RenderStopsNames(const std::map<std::string_view, transport::Stop*>& list_stops, const SphereProjector& sp) const {
