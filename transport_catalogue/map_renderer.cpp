...
svg::Document MapBuilder::BuildSvg(const transport::TransportCatalogue& catalogue){
    svg::Document svg_doc;
    
    const std::map<std::string_view, transport::TransportCatalogue::Bus*> list_buses = catalogue.GetBusCatalogue();
    std::map<std::string_view, transport::TransportCatalogue::Stop*> list_stops;
    std::vector<geo::Coordinates> stops_crd;
    
    for (const auto& [bus_number, bus] : list_buses) {
        if (bus->GetStopsPtrs(catalogue).empty()) continue;
        for (const auto& stop : bus->GetStopsPtrs(catalogue)) {
            stops_crd.push_back(stop->point);
            list_stops[stop->name] = stop;
        }
    }
...
std::vector<svg::Polyline> MapBuilder::RouteOutline(const transport::TransportCatalogue& catalogue, const SphereProjector& sp) const{
    const std::map<std::string_view, transport::TransportCatalogue::Bus*> list_buses = catalogue.GetBusCatalogue();
    ...
    for (const auto& [bus_number, bus] : list_buses) {
        if (bus->GetStopsPtrs(catalogue).empty()) continue;
        std::vector<transport::TransportCatalogue::Stop*> route_stops= bus->GetStopsPtrs(catalogue);
        if (bus->cycle == false ) {
                auto it=route_stops.end();
                route_stops.insert(it, std::next(route_stops.rbegin()), route_stops.rend());
            }
        svg::Polyline line;    
...
std::vector<svg::Text> MapBuilder::RenderBusLabel(const transport::TransportCatalogue& catalogue, const SphereProjector& sp) const {
    ...
    size_t color_num = 0;
    for (const auto& [bus_number, bus] :list_buses) {
        std::vector<transport::TransportCatalogue::Stop*> route_stops_ptrs = bus->GetStopsPtrs(catalogue);
        if (bus->stops.empty()) continue;
        svg::Text text;
        svg::Text underlayer;
        text.SetPosition(sp(route_stops_ptrs[0]->point));
        text.SetOffset(settings.bus_label_offset);
        text.SetFontSize(settings.bus_label_font_size);
        text.SetFontFamily("Verdana");
        text.SetFontWeight("bold");
        text.SetData(bus->num);
        text.SetFillColor(settings.color_palette[color_num]);
        if (color_num < (settings.color_palette.size() - 1)) ++color_num;
        else color_num = 0;
        
        underlayer.SetPosition(sp(route_stops_ptrs[0]->point));
        ...
        if (bus->cycle == false && route_stops_ptrs[0] != route_stops_ptrs[route_stops_ptrs.size() - 1]) {
            svg::Text text2 {text};
            svg::Text underlayer2 {underlayer};
            text2.SetPosition(sp(route_stops_ptrs[route_stops_ptrs.size() - 1]->point));
            underlayer2.SetPosition(sp(route_stops_ptrs[route_stops_ptrs.size() - 1]->point));
...
std::vector<svg::Circle> MapBuilder::RenderStopsCircles(const std::map<std::string_view, transport::TransportCatalogue::Stop*>& list_stops, const SphereProjector& sp) const {
    ...
    std::vector<svg::Circle> circle_container;
        ...
        for (const auto& [name,stop] : list_stops) {
            ...
            svg::Circle circle;
            circle.SetCenter(sp(stop->point));
...
std::vector<svg::Text> MapBuilder::RenderStopsNames(const std::map<std::string_view, transport::TransportCatalogue::Stop*>& list_stops, const SphereProjector& sp) const {
